...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const backdrop = document.querySelector("#ud-notes-modal .ud-modal-backdrop");
  ...
  channel.subscribe("note-done", async ({
    data
  }) => {
    await loadNotes();
  ...
  document.querySelector("#ud-notes-add").addEventListener("click", async () => {
...
async function loadNotes() {
  try {
    ...
    notes.forEach(note => {
      ...
      let html = `
                <div class="ud-note">
                    <strong>${note.author || "Unbekannt"}:</strong> ${note.message}
                </div>
            `;
                    ...
                    <div class="ud-note-replies">
                        ${note.replies.map(r => `
                                <div class="ud-note-reply">
                                    <strong>${r.author || "Unbekannt"}:</strong> ${r.message}
                                </div>
                            `).join("")}
                    </div>
                `;
...
<path d="M9.5501 18L3.8501 12.3L5.2751 10.875L9.5501 15.15L18.7251 5.97498L20.1501 7.39998L9.5501 18Z" fill="black"/>
 ...
 Erledigt</button>
      ...
      item.querySelector(".ud-note-done").addEventListener("click", async e => {
      ...
      item.querySelector(".ud-note-reply-btn").addEventListener("click", async e => {
